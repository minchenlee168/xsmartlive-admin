...
const name = ref('')
/** 區間 DatePicker：[起, 迄]；任一空都允許（送出時各自轉成 Date | null） */
...
function onCreate(): void {
  if (!name.value.trim()) { hasError.value = true; return }
  emit('create', {
    ...
    <div class="flex flex-col gap-4 pt-1">
      <div class="flex flex-col gap-2">
        ...
        <InputText
          ...
          class="w-full"
          :invalid="hasError && !name.trim()"
          @keyup.enter="onCreate"
        />
        <span v-if="hasError && !name.trim()" class="text-[12px] text-[#dc2626]">請輸入貼文收單名稱</span>
      </div>
